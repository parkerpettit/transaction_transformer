...
from evaluation.evaluate import evaluate            # per-feature val metrics

import yaml
from utils.utils import load_cfg, merge, load_ckpt, save_ckpt
...
def main():
    ...
    # ── Training mode ────────────────────────────────────────────────────────────
    ap.add_argument("--mode", type=str, default="ar", choices=["ar", "masked", "mlm"], 
    ...
    ap.add_argument("--mask_prob", type=float, default=0.15, 
                    help="Probability of masking tokens in MLM mode")
        ...
        for ep in range(start_epoch, args.total_epochs):
            ...
            for batch in prog_bar:
                ...
                if args.use_mixed_precision and scaler is not None:
                    with autocast():
                        if args.mode in ["masked", "mlm"]:
                            # MLM training mode
                            B, L = cat_input.shape[:2]
                            
                            # Create mask
                            mask = create_mlm_mask(B, L, args.mask_prob, device)
                            
                            # Apply masking to inputs
                            cat_masked = apply_categorical_masking(cat_input, mask, 
                                                                 {k: len(enc[k]["inv"]) for k in cat_features})
                            cont_masked = apply_continuous_masking(cont_inp, mask)
                            
                            # Forward pass with masked inputs
                            cat_logits, cont_pred = model(cat_masked, cont_masked, mode="masked")
                            
                            # Compute MLM loss
                            loss, loss_cat, loss_cont = compute_mlm_loss(
                                cat_logits, cont_pred, cat_input, cont_inp, mask,
                                {k: len(enc[k]["inv"]) for k in cat_features}, cat_features
                            )
