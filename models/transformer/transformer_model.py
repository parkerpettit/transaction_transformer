...
class TransactionModel(nn.Module):
    ...
    def __init__(self, cfg: ModelConfig):
        ...
        # -- Autoregressive heads (predict next transaction)
        ...
        # -- MLM heads (predict masked tokens)
    ...
    def forward(
        ...
        if mode == "masked" or mode == "mlm":
            # For MLM, we predict all sequence positions
            z = self.mlm_dropout(seq)  # (B, L, D)
            cat_logits = self.mlm_cat_head(z)  # (B, L, total_cat_vocab)
            cont_pred = self.mlm_cont_head(z)  # (B, L, num_cont_features)
            return cat_logits, cont_pred
