...
from transaction_transformer.config.config import ModelConfig   # only for typing / pretty storage
...
def load_ckpt(
    ...
    model = FeaturePredictionModel(config=ckpt["config"], schema=schema).to(device)
...
) -> Tuple[Any, float, int, torch.optim.AdamW]:
    ckpt = torch.load(path, map_location=device, weights_only=False)
    config = ckpt["config"]
    from transaction_transformer.modeling.models import FeaturePredictionModel
    model = FeaturePredictionModel(config=config, schema=schema).to(device)
    model.load_state_dict(ckpt["model_state"], strict=False)
