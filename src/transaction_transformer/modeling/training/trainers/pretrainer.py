...
class Pretrainer(BaseTrainer):
    ...
    def __init__(
        ...
        # Mixed precision (AMP)
        use_amp = bool(config.model.training.mixed_precision) and str(device).startswith("cuda")
    ...
    def compute_unified_loss(
        ...
        for f_idx, name in enumerate(self.schema.cont_features):
            ...
            if mask.any():
                ...
                # Match dtype to logits for AMP compatibility
                soft_targets = soft_targets.to(dtype=logits_flat.dtype)
    ...
    def train_epoch(self) -> Dict[str, float]:
        ...
        for batch in self.train_bar:
            # if batch_idx == 50:
            #     break
            with self.autocast("cuda", enabled=self.autocast_enabled):
