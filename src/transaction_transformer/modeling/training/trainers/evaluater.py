...
class Evaluater:
    ...
    def forward_pass(self, model: FraudDetectionModel, batch: Dict[str, torch.Tensor]) -> Tuple[torch.Tensor, torch.Tensor]:
    ...
    def evaluate(self) -> None:
        for i, model_path in enumerate(self.model_paths):
            ...
            self.metrics.wandb_run = wandb.init(project=self.config.metrics.wandb_project, name=self.config.metrics.run_name, config=self.config.to_dict(), tags=[self.config.model.training.model_type])
            if self.metrics.wandb_run:
                artifact = self.metrics.wandb_run.use_artifact(model_path)
                artifact_dir = Path(artifact.download())
                backbone = torch.load(artifact_dir / "backbone.pt", map_location="cpu", weights_only=False)
                model.backbone.load_state_dict(backbone["state_dict"], strict=True)
                head = torch.load(artifact_dir / "clf_head.pt", map_location="cpu", weights_only=False)
                model.head.load_state_dict(head["state_dict"], strict=True)
            else:
                raise ValueError("[Evaluater] Warning: no wandb run found, please specify correct model path in evaluater.py")
            model.to(self.device)
            model.eval()

            # Validate for one epoch
            self.metrics.start_epoch()
            self.val_bar = tqdm(self.val_loader, desc=f"Validation {model_path}", bar_format=self.bar_fmt, leave=True)
