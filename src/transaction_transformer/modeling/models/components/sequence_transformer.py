...
class SinCosPositionalEncoding(nn.Module):
    ...
    def forward(self, x: Tensor) -> Tensor:
        ...
        if self.pe.numel() == 0 or self.pe.size(1) < L or self.pe.device != x.device or self.pe.dtype != x.dtype:
...
class SequenceTransformer(nn.Module):
    ...
    def forward(
        ...
        *,
        ...
        return self.encoder(x, mask=attn_mask, src_key_padding_mask=key_padding_mask) # (B, L, M)
