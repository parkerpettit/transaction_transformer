...
class FrequencyEncoder(nn.Module):
    ...
    def forward(self, v: torch.Tensor) -> torch.Tensor:
        ...
        return torch.stack((sin, cos), dim=-1).reshape(
            *x.shape[:-1], 2 * self.L
        )  # (B, L, F, 2L)
...
class EmbeddingLayer(nn.Module):
    ...
    def __init__(
        ...
        self.cat_emb = nn.ModuleList(
            [
                nn.Embedding(
                    self.cat_encoders[f].vocab_size,
                    self.emb_dim,
        ...
        self.cont_proj = nn.ModuleList(
            [
                nn.Linear(2 * self.freq_enc.L, self.emb_dim, bias=False)
                for _ in self.cont_features
            ]
        )

        # Dropout
        self.dropout = (
            nn.Dropout(self.dropout_float) if self.dropout_float > 0 else nn.Identity()
        )
    ...
    def forward(self, cat: torch.LongTensor, cont: torch.Tensor) -> torch.Tensor:
        ...
        # --- Categorical fields ---
        cat_embs = [
            emb(cat[..., i]) for i, emb in enumerate(self.cat_emb)
        ]  # each (B, L, D)
        ...
        for i, proj in enumerate(self.cont_proj):
            ...
            y = proj(gamma)  # (B, L, D)
            if m.any():
                # Match dtype/device under AMP autocast to avoid dtype mismatch on assignment
                y[m] = self.mask_token.to(device=y.device, dtype=y.dtype)  # replace masked slots
            cont_embs.append(y)
        ...
        return embs_stacked.reshape(
            B * L, self.num_fields, self.emb_dim
        )  # (B*L, K, D),
