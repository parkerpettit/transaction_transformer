...
def main():
    ...
    parser.add_argument(
        ...
        help="Path to config YAML (relative to config dir)",
    )
    parser.add_argument(
        "--artifact-name",
        type=str,
        default="raw-card-transactions",
        help="Artifact name",
    )
    parser.add_argument(
        "--project", type=str, default=None, help="W&B project override"
    )
    ...
    run = wandb.init(
        project=project, job_type="upload_raw_data", name="upload_raw_data"
    )
    ...
    art = wandb.Artifact(
        name=args.artifact_name, type="dataset", description="Raw card transactions CSV"
    )
