...
class TxnDataset(Dataset):
    ...
    def __init__(
        ...
        cat_np   = df[cat_features].to_numpy(dtype=np.int64)
        cont_np  = df[cont_features].to_numpy(dtype=np.float32)
        ...
        # 2) compute your group offsets exactly as before
...
def extract_embeddings(
    ...
    # 1) metadata
    N = len(ds)
    M = pretrained_model.cfg.seq_config.d_model # type: ignore
    # save meta
